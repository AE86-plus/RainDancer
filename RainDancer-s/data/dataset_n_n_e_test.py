...
import os
import sys
from data .spatial_transform import Random_crop ,ToTorchFormatTensor ,Center_crop
...
class SequenceDataset (Dataset ):
    ...
    def __init__ (self ,h5_file ,seq_len ,scene_type ,transform_flag =True ):
        ...
        if self .h5_format =="rainsyn":
            frame_count =len ([k for k in self .h5 ["input"][scene_type ].keys ()if k !="voxel"])
            self .num_sequences =frame_count -seq_len +1
        else :
    ...
    def get_sequence (self ,A_idx ):
        ...
        for i in range (self .seq_len ):
            if self .h5_format =="rainsyn":
                rainy_frame .append (self .h5 ["input/{}/{:05d}".format (self .scene_type ,A_idx +i )][:][:,:,::-1 ])
                clean_frame .append (self .h5 ["processed/{}/{:05d}".format (self .scene_type ,A_idx +i )][:][:,:,::-1 ])
            else :
        ...
        for i in range (self .seq_len -1 ):
            if self .h5_format =="rainsyn":
                rainy_event .append (self .h5 ["input/{}/voxel/{:05d}".format (self .scene_type ,A_idx +i )][:])
            else :
