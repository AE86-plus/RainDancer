...
from networks .dclgan_util .image_pool import ImagePool
from networks .basic import networks as sub_networks
from networks .basic .base_model import BaseModel
from networks .basic .loss_FA import FALoss_max ,FALoss_min ,FALoss_max_NA
from networks .basic .patchnce import PatchNCELoss_Layer as PatchNCELoss
...
class EventBranchLoss (nn .Module ):
    ...
    def forward (self ,event_bg ,gt_event ,return_parts =False ):
        ...
        """
        assert event_bg .shape ==gt_event .shape ,"event_bg and gt_event must have the same shape"
...
class CrossModalFusion_R (nn .Module ):
    def __init__ (self ,channels_x1 =96 ,channels_x1_e =96 ,reduction_ratio =4 ,out_channels =32 ):
        super (CrossModalFusion_R ,self ).__init__ ()

        total_channels =channels_x1 +channels_x1_e
        reduced_channels =total_channels //reduction_ratio


        self .pre_conv =nn .Conv2d (total_channels ,reduced_channels ,kernel_size =1 )


        self .branch7 =nn .Sequential (
        nn .Conv2d (reduced_channels ,reduced_channels ,kernel_size =7 ,padding =3 ,groups =reduced_channels ),
        ...
        _make_gn (out_channels ,8 ),
        nn .Sigmoid ()
        )

        self .residual_conv =nn .Conv2d (total_channels ,out_channels ,kernel_size =1 )
    ...
    def _init_weights (self ):
        for m in self .modules ():
            ...
            elif isinstance (m ,(nn .BatchNorm2d ,nn .BatchNorm3d ,nn .SyncBatchNorm ,nn .GroupNorm )):
    ...
    def forward (self ,x1_r ,x1_e_r ):
        ...
        x_cat =torch .cat ([x1_r ,x1_e_r ],dim =1 )


        x_reduced =self .pre_conv (x_cat )


        out7 =self .branch7 (x_reduced )
        out5 =self .branch5 (x_reduced )
        out3 =self .branch3 (x_reduced )


        combined =torch .cat ([out7 ,out5 ,out3 ],dim =1 )
        fused =self .final_conv (combined )
...
class RGB_Extracted (nn .Module ):
    ...
    def __init__ (self ,channels =96 ):
        ...
        self .CSA =multi_Sym_CBAM (gate_channels =channels ,n_blocks =4 )


        #self.bn = nn.BatchNorm2d(channels)
        self .norm =_make_gn (channels ,8 )
        self .act =nn .PReLU ()
...
class SNN_Extracted (nn .Module ):
    def __init__ (self ,args ,out_channels =96 ,event_channels =20 ):
        super ().__init__ ()
        ...
        nn .Conv2d (event_channels ,out_channels ,kernel_size =3 ,padding =1 ,bias =False ),
        _gn (out_channels ,8 ),
        ...
        self .t_pool =nn .Conv3d (8 ,1 ,kernel_size =1 ,bias =True )
        ...
        self .CSA =multi_Sym_CBAM (gate_channels =out_channels ,n_blocks =4 )
    ...
    def forward (self ,x_e :torch .Tensor ,x_e_r ):
        ...
        """
        B ,C ,H ,W =x_e .shape
        assert C ==self .event_channels ,f"x_e channel count must be {self .event_channels } (pre10 + post10)"


        x_base =self .base_proj (x_e )# (B,out_channels,H,W)


        x =x_e .view (B ,1 ,self .event_channels ,H ,W )# (B,1,event_channels,H,W)


        x =self .pre_spa (x )# (B,1,20,H,W)


        x =self .t_down (x )# (B,1,10,H,W)


        x =self .conv1 (x )# (B,8,10,H,W)


        x =self .s_down2 (self .s_down1 (x ))# (B,8,10,H/4,W/4)


        x =x .permute (2 ,0 ,1 ,3 ,4 ).contiguous ()# (T=10,B,8,H/4,W/4)
        self .reset_state ()
        x =self .block (x )# (T,B,8,H/4,W/4)
        x =x .permute (1 ,2 ,0 ,3 ,4 ).contiguous ()# (B,8,10,H/4,W/4)


        x =self .post_low (x )# (B,8,10,H/4,W/4)



        x =F .interpolate (x ,scale_factor =(1 ,4 ,4 ),mode ="trilinear",align_corners =False )# (B,8,10,H,W)


        x =self .post_3d (x )# (B,8,10,H,W)


        w =torch .softmax (self .t_pool (x ),dim =2 )# (B,1,10,H,W)
        x2d =(x *w ).sum (dim =2 )# (B,8,H,W)



        rain_pred =self .rain_head (x2d )# (B,out_channels,H,W)
        B ,C ,H ,W =x_e_r .shape
        if C ==self .out_channels :
            rain_pred =rain_pred +x_e_r
        rain_pred =self .CSA (rain_pred )# (B,out_channels,H,W)


        a =torch .sigmoid (self .alpha )
        x_e_r =a *rain_pred # (B,out_channels,H,W)
        x_e_b =x_base -x_e_r # (B,out_channels,H,W)
...
class Head_F (nn .Module ):
    ...
    def __init__ (self ,args ):
        ...
        feature_channels =nf *sequence_length
        use_bias =True
        args ["norm"]='ln'
        ...
        self .conv1 =ConvLayer (3 *sequence_length ,feature_channels ,kernel_size =3 ,stride =1 ,groups =sequence_length ,bias =use_bias ,norm =args ["norm"])
        self .res1 =ResidualBlock (feature_channels ,groups =sequence_length ,bias =use_bias ,norm =args ["norm"])
        ...
        self .conv2 =nn .Conv3d (nf ,nf ,kernel_size =(3 ,3 ,3 ),stride =(1 ,1 ,1 ),padding =(1 ,1 ,1 ),bias =True )
        #self.bn2 = nn.BatchNorm3d(nf)
        self .norm2 =_make_gn (nf ,8 )
        self .relu =nn .LeakyReLU (negative_slope =0.2 ,inplace =True )
        self .res2 =ResidualBlock (feature_channels ,groups =1 ,bias =True ,norm =args ["norm"])
    ...
    #print(rainy_frame.shape)[8,9,128,128]
        ...
        X1_3d =To3D (x1 ,self .sequence_length )
        x1 =self .conv2 (X1_3d )
        ...
        X1 =self .res2 (self .relu (x1 ))#[B, feature_channels, H, W]
...
class MultiStageFusion (nn .Module ):
    def __init__ (self ,n_stages =3 ,feature_channels =96 ,rain_channels =32 ,event_channels =20 ,bimodal_hidden =None ,detach_reset =True ,crm_num_crb =2 ,crm_num_rcab =2 ):
        super (MultiStageFusion ,self ).__init__ ()
        self .n_stages =n_stages
        self .feature_channels =feature_channels
        self .rain_channels =rain_channels
        self .crm_num_crb =crm_num_crb
        self .crm_num_rcab =crm_num_rcab
        if bimodal_hidden is None :
            bimodal_hidden =feature_channels


        self .shared_stage =nn .ModuleList ([

        RGB_Extracted (channels =feature_channels ),

        SNN_Extracted (args ={"detach_reset":detach_reset ,},out_channels =feature_channels ,event_channels =event_channels ),

        CRM (n_feat =feature_channels ,num_crb =crm_num_crb ,num_rcab =crm_num_rcab ),

        CRM (n_feat =feature_channels ,num_crb =crm_num_crb ,num_rcab =crm_num_rcab ),

        CrossModalFusion_R (channels_x1 =feature_channels ,channels_x1_e =feature_channels ,out_channels =rain_channels ),

        #BiModalComplementaryFusion(c=96, mode="interaction", detach_other=True),
        BiModalComplementaryFusion (c =feature_channels ,hidden =bimodal_hidden ),
        ])
...
class RMFD (BaseModel ):
    ...
    def __init__ (self ,args ,opt ,local_rank ):
        ...
        self .sequence_length =self .args .get ("sequence_length",3 )
        self .nf =self .args .get ("nf",32 )
        self .feature_channels =self .nf *self .sequence_length
        self .rain_channels =self .nf
        self .event_channels =self .args .get ("event_input_nc",self .args .get ("num_bins",10 )*2 )
        self .bimodal_hidden =self .args .get ("bimodal_hidden",self .feature_channels )
        self .crm_num_crb =self .args .get ("crm_num_crb",2 )
        self .crm_num_rcab =self .args .get ("crm_num_rcab",2 )
        ...
        self .stage =MultiStageFusion (
        n_stages =3 ,
        feature_channels =self .feature_channels ,
        rain_channels =self .rain_channels ,
        event_channels =self .event_channels ,
        bimodal_hidden =self .bimodal_hidden ,
        detach_reset =self .args .get ("detach_reset",True ),
        crm_num_crb =self .crm_num_crb ,
        crm_num_rcab =self .crm_num_rcab ,
        ).to (local_rank )

        self .rain_con =RainFusionModule (n_stages =3 ,c_each =self .rain_channels ,out_channels =self .feature_channels ).to (local_rank )

        self .recovery =Recovery (n_feat =self .feature_channels ).to (local_rank )


        self .decoder1 =Decoder (isrgb =True ,input_channels =self .feature_channels ,output_channels =3 ).to (local_rank )
        self .decoder2 =Decoder (isrgb =False ,input_channels =self .feature_channels ,output_channels =self .event_channels ).to (local_rank )
        self .decoder3 =Decoder (isrgb =True ,input_channels =self .feature_channels ,output_channels =3 ).to (local_rank )
