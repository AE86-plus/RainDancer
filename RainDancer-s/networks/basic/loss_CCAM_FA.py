...
class SimMaxLoss_twostream(nn.Module):
    def __init__(self, metric='cos', alpha=0.25, reduction='mean'):
        super(SimMaxLoss_twostream, self).__init__()
        self.metric = metric
        self.alpha = alpha
        self.reduction = reduction

    def forward(self, embedded_bg, embedded_bg_1):
        """
        :param embedded_fg: [N, C]
        :param embedded_bg: [N, C]
        :return:
        ...
        if self.metric == 'l2':
            ...
            sim = cos_simi(embedded_bg, embedded_bg_1)
            loss = -torch.log(sim)
            loss[loss < 0] = 0
            _, indices = sim.sort(descending=True, dim=1)
            _, rank = indices.sort(dim=1)
...
if __name__ == '__main__':
    ...
    pos_loss_contrast = SimMaxLoss_twostream(metric='cos').cuda()
    embedded_bg, embedded_fg = torch.randn(2048, 256).cuda(), torch.randn(2048, 256).cuda()
    loss = pos_loss_contrast(embedded_bg, embedded_fg)
