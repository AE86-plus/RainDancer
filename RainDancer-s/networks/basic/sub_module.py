...
class ChannelGate(nn.Module):
    ...
    def forward(self, x):
        ...
        scale = F.sigmoid( channel_att_sum ).unsqueeze(2).unsqueeze(3).expand_as(x) #channel_att_sum.shape = [1,32], scale.shape = [1,32,64,64]
        return x * scale
...
if __name__ == "__main__":
    ...
    input = torch.randn(1,5,64,64,64).cuda()
    tsa_model = TSAFusion().cuda()
    # CA_module = ChannelAttention(32, 16).cuda()
    # CA_module_1 = ChannelGate(32, 16).cuda()
    
    # SA_module = SpatialGate().cuda()
    # SA_module_1 = SpatialAttention().cuda()
    
    num = count_network_parameters(tsa_model)
    # num_1 = count_network_parameters(SA_module_1)
