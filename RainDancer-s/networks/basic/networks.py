...
import numpy as np
from networks .basic .stylegan_networks import StyleGAN2Discriminator ,StyleGAN2Generator ,TileStyleGAN2Discriminator
from networks .basic .moco import MoCo
import sys
...
class Downsample (nn .Module ):
    def __init__ (self ,channels ,pad_type ='zero',filt_size =3 ,stride =2 ,pad_off =0 ):
...
def init_net_mgpu (net ,init_type ='normal',init_gain =0.02 ,gpu_ids =[],debug =False ,initialize_weights =True ):
    """Initialize a network: 1. register CPU/GPU device (with multi-GPU support); 2. initialize the network weights
    Parameters:
        net (network)      -- the network to be initialized
        init_type (str)    -- the name of an initialization method: normal | xavier | kaiming | orthogonal
    ...
    # print(f"init_type is {init_type}, init_gain is {init_gain}, debug is {debug}")
        init_weights (net ,init_type ,init_gain =init_gain ,debug =debug )
    return net
...
init_gain =0.02 ,no_antialias =False ,no_antialias_up =False ,gpu_ids =[],opt =None ):
    ...
    return init_net_mgpu (net ,init_type ,init_gain ,gpu_ids ,initialize_weights =('stylegan2'not in netG ))


def define_F (input_nc ,netF ,norm ='batch',use_dropout =False ,init_type ='normal',init_gain =0.02 ,no_antialias =False ,gpu_ids =[],opt =None ,local_rank =0 ):
    if netF =='global_pool':
        net =PoolingF ()
    elif netF =='reshape':
        net =ReshapeF ()
        ...
        net =PatchSampleF (use_mlp =False ,init_type =init_type ,init_gain =init_gain ,gpu_ids =gpu_ids ,nc =opt .netF_nc ,local_rank =local_rank )
    elif netF =='mlp_sample':
        net =PatchSampleF (use_mlp =True ,init_type =init_type ,init_gain =init_gain ,gpu_ids =gpu_ids ,nc =opt .netF_nc ,local_rank =local_rank )
    elif netF =='strided_conv':
        net =StridedConvF (init_type =init_type ,init_gain =init_gain ,gpu_ids =gpu_ids )
    else :
        raise NotImplementedError ('projection model name [%s] is not recognized'%netF )
    return init_net_mgpu (net ,init_type ,init_gain ,gpu_ids )
...
def define_D (input_nc ,ndf ,netD ,n_layers_D =3 ,norm ='batch',init_type ='normal',init_gain =0.02 ,no_antialias =False ,gpu_ids =[],opt =None ):
    ...
    net =None
    norm_layer =get_norm_layer (norm_type =norm )
    # nn.InstanceNorm2d(get_out_channel(layer), affine=False)
    # norm_layer = nn.InstanceNorm2d(affine=False)

    if netD =='basic':# default PatchGAN classifier
        net =NLayerDiscriminator (input_nc ,ndf ,n_layers =3 ,norm_layer =norm_layer ,no_antialias =no_antialias ,)
    elif netD =="cl_basic":
        net =CL_NLayerDiscriminator (opt ,input_nc ,ndf ,n_layers =3 ,norm_layer =norm_layer ,no_antialias =no_antialias ,)
...
class PatchSampleF (nn .Module ):
    def __init__ (self ,use_mlp =False ,init_type ='normal',init_gain =0.02 ,nc =256 ,gpu_ids =[],local_rank =0 ):
    # potential issues: currently, we use the same patch_ids for multiple images in the batch
        ...
        # print("In PatchSmapleF, the device is cuda:{}".format(local_rank))

    def create_mlp (self ,feats ):
        for mlp_id ,feat in enumerate (feats ):
            input_nc =feat .shape [1 ]
            mlp =nn .Sequential (*[nn .Linear (input_nc ,self .nc ),nn .ReLU (),nn .Linear (self .nc ,self .nc )]).to (self .local_rank )
            # if len(self.gpu_ids) > 0:
            #     mlp.cuda()
            setattr (self ,'mlp_%d'%mlp_id ,mlp )
        init_net_mgpu (self ,self .init_type ,self .init_gain ,self .gpu_ids )
        self .mlp_init =True
...
class PatchSampleF_layer (nn .Module ):
    def __init__ (self ,use_mlp =False ,init_type ='normal',init_gain =0.02 ,nc =256 ,gpu_ids =[],local_rank =0 ):
    # potential issues: currently, we use the same patch_ids for multiple images in the batch
        super (PatchSampleF ,self ).__init__ ()
        self .l2norm =Normalize (2 )
        ...
        for feat_id ,feat in enumerate (feats ):
            ...
            if num_patches >0 :
                ...
                # print(feats[0].device)
                    ...
                    patch_id_key =torch .randperm (feat_reshape .shape [1 ],device =feats [0 ].device )
                    patch_id_key =patch_id_key [:int (min (num_patches ,patch_id_key .shape [0 ]))]

                x_sample =feat_reshape [:,patch_id ,:].flatten (0 ,1 )# reshape(-1, x.shape[1])
                x_sample_key =feat_reshape [:,patch_id_key ,:].flatten (0 ,1 )
...
class CL_NLayerDiscriminator (nn .Module ):

    def __init__ (self ,opts ,input_nc =3 ,ndf =64 ,n_layers =3 ,norm_layer =nn .BatchNorm2d ,no_antialias =False ):
        super (CL_NLayerDiscriminator ,self ).__init__ ()

        dim =256

        self .E =MoCo (base_encoder =NLayerDiscriminator ,opt =opts ,dim =dim ,K =opts .batch_size *dim ,input_nc =input_nc ,ndf =64 ,n_layers =3 ,norm_layer =nn .BatchNorm2d ,no_antialias =False )

    def forward (self ,x_query ,x_key ):

        logits ,labels ,dis_out =self .E (x_query ,x_key )

        return logits ,labels ,dis_out
